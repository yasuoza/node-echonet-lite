...
var EchonetLite = function (params) {
	...
	if (this.type === 'lan') {
		...
		if ('membership' in params) {
			...
			if (typeof (membership) === 'boolean') {
				...
				throw new Error('The value of the "membership" property must be boolean.');
			}
		}
	...
	this.custom_transaction_id = null;
...
EchonetLite.prototype._addCallback = function (callback, once) {
	var tid = this.custom_transaction_id;
	if(tid === null) {
		tid = this._getNewTransactionId();
	}
	this.callbacks[tid] = {
		'once': once,
		'callback': callback
...
EchonetLite.prototype.send = function (address, eoj, esv, prop, callback) {
	...
	var tid = this._addCallback(callback, true);
	var data = {
	...
	if (this.timeout > 0) {
		setTimeout(function(tid) {
			...
			this._execCallback(tid, new Error("Request timeout"), null);
		}.bind(this), this.timeout, tid)
	}
};


/* ------------------------------------------------------------------
* Method: setTransactionId(id)
* ---------------------------------------------------------------- */
EchonetLite.prototype.setTransactionId = function (id) {
	if(id === null) {
		this.custom_transaction_id = null;
	} else if(typeof(id) === 'number') {
		if(id % 1 === 0 && id >= 0 && id <= 0xffff) {
			this.custom_transaction_id = id;
		} else {
			throw new Error('The transaction ID must be an integer in the range of 0x00 to 0xffff.');
		}
	} else {
		throw new Error('The transaction ID must be an integer.');
	}
};
